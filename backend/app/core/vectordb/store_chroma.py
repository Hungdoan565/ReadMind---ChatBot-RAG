...
def hybrid_search(
    ...
    # Sort by fused RRF score descending. Fuse ALL candidates (do NOT truncate
    # to top_k here) so rerank sees the full fused pool (up to fetch_k). Slicing
    # to final_n happens AFTER reranking (Req 7).
    ...
    logger.debug(
        f"Hybrid search: {len(dense_docs)} dense + {len(bm25_ranked)} BM25 "
        f"-> {len(fused)} fused -> {len(result)} final"
    )
    return result
...
def get_hybrid_retriever(
    ...
    filter_room_code = room_code  # Capture in closure
